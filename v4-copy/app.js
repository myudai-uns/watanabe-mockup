...
const fmt = {
  ...
  paperText(it) { if (!it) return '—'; const m = (it.paper_other_memo || '').trim(); return m || this.paper(it.paper_id); },
    ...
    return `
      ...
      <div class="grid grid-cols-5 gap-3 mb-6">
        ...
        <a href="#orders?filter=overdue" class="dash-card bg-white border-2 border-red-700 p-4 rounded shadow-sm hover:shadow-md transition" data-filter="overdue">
          <div class="text-xs text-ink-500 font-bold">納期経過</div>
        ...
        <div class="bg-white border-2 border-blue-500 p-4 rounded shadow-sm">
          <div class="text-xs text-ink-500 font-bold">今月売り上げ見込み</div>
      ...
      <div class="grid grid-cols-3 gap-4">
            ...
            <tbody>
              ${todayDue.length === 0 ? `<tr><td colspan="5" class="text-center py-6 text-ink-500">本日納品予定はありません</td></tr>`
                ...
                <tr class="border-t hover:bg-brand/5 cursor-pointer" onclick="location.hash='#order/${o.id}'">
                  ...
                  <td class="px-3 py-2 text-xs">${esc(fmt.paperText(o.items[0]))} ×${o.items[0]?.quantity}</td>
        ...
        <div class="bg-white rounded shadow-sm">
          <div class="text-white px-4 py-2 rounded-t font-bold" style="background:#C0392B">最近の変更履歴</div>
            ...
            ${filtered.length === 0 ? `<tr><td colspan="8" class="text-center py-10 text-ink-500">該当なし</td></tr>` :
              pageList.map(o => {
                ...
                <tr class="border-t hover:bg-brand/5 cursor-pointer" onclick="location.hash='#order/${o.id}'">
                  ...
                  <td class="px-3 py-2">${o.delivered_date ? esc(fmt.dateW(o.delivered_date)) : esc(fmt.delivery(o))}${overdue ? `<div class="text-xs text-purple-600 font-bold mt-1">${overdue.replace('経過','超過')}</div>` : ''}</td>
...
Screens.order_new = {
            ...
            <ul class="text-sm divide-y max-h-96 overflow-auto">
              ${pastOrders.length === 0 ? `<li class="px-3 py-4 text-ink-500 text-center text-xs">過去発注なし</li>` :
                ...
                <li class="px-3 py-2 hover:bg-ok/5">
                  ...
                  <div class="text-xs text-ink-500">${esc(fmt.paperText(o.items[0]))} ${o.items[0]?.quantity}枚 / ${fmt.money(o.total_amount)}</div>
    ...
    return `
      <div class="p-4 ${i > 0 ? 'border-t' : 'border-t'}" data-item-idx="${i}">
        <div class="flex items-center gap-2 mb-2">
          <span class="text-xs font-black bg-ink-900 text-white px-2 py-0.5 rounded">明細 #${i+1}</span>
        ...
        <div class="grid grid-cols-6 gap-2 text-sm">
          <div class="col-span-2">
            ...
            <input type="text" class="w-full border rounded px-2 py-1.5 item-paper-free" data-idx="${i}" placeholder="用紙（自由入力 例: コート135kg 白 A4）" value="${esc(it.paper_other_memo || (it.paper_id ? fmt.paper(it.paper_id) : ''))}">
          </div>
          <div>
  ...
  bind() {
    ...
    this.bindItemHandlers = () => {
      ...
      $$('.item-paper-free').forEach(el => el.addEventListener('input', (e) => { const it = d.items[+e.target.dataset.idx]; it.paper_other_memo = e.target.value; it.paper_id = ''; }));
              ...
              <tbody>
                ${o.items.map((it, i) => {
                  ...
                  <tr class="border-t">
                    ...
                    <td class="px-3 py-2">${esc(fmt.paperText(it))}</td>
        ...
        <div class="space-y-2 text-sm">
          ...
          <div class="border rounded p-2 grid grid-cols-12 gap-2" data-eitem="${i}">
            <div class="col-span-5">
              <input type="text" class="w-full border rounded px-2 py-1 e-paper" data-i="${i}" placeholder="用紙（自由入力）" value="${esc(it.paper_other_memo || (it.paper_id ? fmt.paper(it.paper_id) : ''))}">
            </div>
            <div class="col-span-2">
    ...
    `, () => {
      ...
      $('#e-save').addEventListener('click', () => {
        const items = o.items.map((it, i) => ({
          ...it,
          paper_other_memo: $(`.e-paper[data-i="${i}"]`).value,
          paper_id: '',
        ...
        <div class="bg-white rounded shadow-sm p-4" id="print-target">
          ...
          <div class="border-2 border-ink-300/50 p-6 bg-white text-xs print-page" id="quote-pdf-page">
            ...
            <div class="flex justify-between mb-4">
              ...
              <img src="assets/corp_block.png" alt="有限会社 渡辺謄写堂" class="block w-[300px] max-w-full self-start">
            </div>
            ...
            ${included.length > 0 ? (() => {
              const specOf = (item) => {
                ...
                return `${esc(fmt.paperText(item))}（${esc(fmt.ink(item.ink_pattern))}）`;
              };
...
Screens.factory = {
  ...
  colStyle(s) {
    return ({
      '見積もり段階': { bg: 'bg-blue-400/20',  text: 'text-blue-800',   border: 'border-blue-500' },
      '受注':     { bg: 'bg-red-500/15',    text: 'text-red-700',    border: 'border-red-500' },
      '印刷中':   { bg: 'bg-yellow-400/30', text: 'text-yellow-700', border: 'border-yellow-500' },
      '完成':     { bg: 'bg-ok/20',         text: 'text-ok-dark',    border: 'border-ok' },
      '納品済み': { bg: 'bg-ink-900/20',    text: 'text-ink-900',    border: 'border-ink-900' },
    ...
    return `
      <div class="mb-4 flex justify-between">
        <div>
          ...
          <p class="text-sm text-ink-500">カードをドラッグしてステータスを変更できます。納期は超過時は「N日経過」と表示。</p>
        </div>
      </div>
            ...
            ${col.items.map((o) => {
              ...
              <div class="bg-white rounded p-3 shadow-sm border-2 ${borderColor} text-sm cursor-pointer kanban-card" draggable="true" data-order-id="${o.id}" onclick="location.hash='#order/${o.id}'">
                <div class="flex justify-between items-baseline gap-2">
                  ...
                  <span class="text-xs whitespace-nowrap">${esc(fmt.delivery(o))}</span>
                </div>
                <div class="font-bold mt-1 truncate">${esc(o.title || fmt.customer(o.customer_id))}</div>
                <div class="text-xs text-ink-500 truncate">${esc(fmt.customer(o.customer_id))}</div>
                ${o.tags && o.tags.length ? `<div class="mt-1">${fmt.tags(o.tags)}</div>` : ''}
              </div>`;
          ...
          <tbody>
            ${list.length === 0 ? `<tr><td colspan="9" class="text-center py-10 text-ink-500">該当なし</td></tr>` :
              ...
              <tr class="border-t hover:bg-brand/5 cursor-pointer" onclick="location.hash='#customer/${c.id}'">
                ...
                <td class="px-3 py-2 font-mono text-xs text-brand">${esc(c.phone || '')}</td>
                <td class="px-3 py-2 font-mono text-xs font-bold">${esc(c.fax || '')}</td>
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
          ...
          <div class="space-y-3 text-sm">
            ...
            <div class="text-right">
              <a href="#password_reset" class="text-xs text-brand hover:underline">パスワードお忘れの方</a>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-sm">
          ...
          <div class="space-y-3 text-sm">
            <div>
              ...
              <input id="rp-pass1" type="text" class="w-full border rounded px-2 py-2" placeholder="••••••••" autocomplete="off">
            </div>
            <div>
              <label class="block text-xs font-bold mb-1">新しいパスワード（確認）</label>
              <input id="rp-pass2" type="text" class="w-full border rounded px-2 py-2" placeholder="••••••••" autocomplete="off">
            </div>
      ...
      <div class="bg-white border-2 border-ink-300/50 p-6 mx-auto print-page" style="width:297mm; max-width:100%; min-height:210mm;">
        ...
        <table class="w-full text-sm border-collapse">
          ...
          <tr>
            ...
            <td class="border p-2 font-bold" colspan="2">${esc(fmt.paperText(it))}</td>
            <td class="border p-2 bg-ink-700/5 font-bold w-20">数量</td>
